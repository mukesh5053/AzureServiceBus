...
namespace ServiceBus.Topics.PaymentEmail
{
    class Program
    {
        ...
        private static async Task SendEmailAsync(Message _message, CancellationToken _token)
        {
            ...
            SharedLib.PaymentEmail email = JsonSerializer.Deserialize<SharedLib.PaymentEmail>(message);
            await Helper.SendPaymentEmail(email);
